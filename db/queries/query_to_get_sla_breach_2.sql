...
base AS (
  SELECT
    ...
    itm.name AS `Issue Type`,
--     itm.name AS issueType,
...
--       END AS isSlaBreach,
    ...
    -- Window function to keep only the latest ticket per (subject, description)
    ROW_NUMBER() OVER (
      PARTITION BY t.subject, t.description
  ...
  WHERE t.reported_date >= '2026-02-01 00:00:00'
)
SELECT
  `Ticket No.`,
  `Module`,
  ...
  `Opening Date & Time` AS reported_date,
  `Current Status`,
  `Resolution Date & Time`,
  `Feedback (if Any)`,
  `SLA Breach (Yes/No)` AS isSlaBreach
FROM base
WHERE rn = 1
ORDER BY `Opening Date & Time` DESC, `Ticket No.` DESC
) 
AS tic
WHERE tic.isSlaBreach = 'Yes'
AND tic.issue_type = 'Problem / Bug'
AND tic.reported_date >= '2026-02-01 00:00:00'
